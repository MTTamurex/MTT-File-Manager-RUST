...
- [ ] Enforce caller-aware authorization on IPC search results before returning full paths.
- [ ] Harden DLL resolution path for WebView2 loader (trusted absolute path + fail-safe policy).
...
- [ ] Make ACL hardening fail-closed for privileged DB/cache paths and harden fallback storage.
