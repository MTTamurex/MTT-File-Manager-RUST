...
use super::{NavigationHistory, ClipboardState, ContextMenuState, WatcherState, RenamingState};
...
impl AppState {
    ...
    pub fn is_renaming(&self, index: usize) -> bool {
        self.renaming_state.as_ref().map_or(false, |s| s.item_index == index)
    }
    ...
    pub fn copy_to_clipboard(&mut self) {
        if let Some(index) = self.selected_item_index {
            if let Some(item) = self.items.get(index) {
                self.clipboard.set(item.path.clone(), super::ClipboardOp::Copy);
            }
        }
    }
    ...
    pub fn cut_to_clipboard(&mut self) {
        if let Some(index) = self.selected_item_index {
            if let Some(item) = self.items.get(index) {
                self.clipboard.set(item.path.clone(), super::ClipboardOp::Move);
            }
        }
    }
    ...
    pub fn increment_generation(&mut self) {
        self.generation += 1;
        self.current_generation.store(self.generation, std::sync::atomic::Ordering::Relaxed);
    }
