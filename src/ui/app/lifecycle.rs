...
pub fn handle_startup_sequence(app: &mut ImageViewerApp, ctx: &egui::Context) {
    if app.startup_tick < STARTUP_REVEAL_TICK {
        ...
        if app.startup_tick == 1 {
            ...
            crate::ui::theme::apply_theme_visuals(ctx, app.theme_mode);
...
pub fn track_window_state(app: &mut ImageViewerApp, ctx: &egui::Context) {
    ...
    if is_focused && !app.was_focused {
        ...
        // Run on a background thread to avoid blocking the UI on a cold
        // NTFS cache (one stat per item in every open tag view).
        app.spawn_purge_missing_tag_views();
    }
    if !is_focused && app.was_focused {
    ...
    if minimized_changed {
        ...
        if is_minimized {
            ...
            app.last_restore_time = std::time::Instant::now();
            app.begin_background_memory_trim();
        } else {
            app.cancel_background_memory_trim();
            ...
            app.run_deferred_post_load_visual_work();
            log::info!(
