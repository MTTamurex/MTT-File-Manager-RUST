...
pub fn render_sidebar(
    ...
    if let Some(onedrive_path) = ctx.onedrive_path {
        ui.horizontal(|ui| {
            ui.add_space(8.0);
            ui.label(egui::RichText::new("Acesso Rápido").size(10.0).color(Color32::from_gray(120)));
        });
        ui.add_space(4.0);
        ...
        if ui.is_rect_visible(rect) {
            ...
            let onedrive_icon = ctx.icon_loader.get_or_load_folder_path_icon(ui.ctx(), onedrive_path);
