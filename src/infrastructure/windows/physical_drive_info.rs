...
use super::physical_drive_identifier::{
    clean_fallback_serial, normalize_translated_nvme_identifier, query_device_id_serial,
};
use super::physical_drive_protocol::{query_nvme_serial, query_usb_ata_firmware};
...
pub fn query_physical_drive_info(drive_letter: char) -> Option<PhysicalDriveInfo> {
    ...
    let info = query_storage_device_property(disk_number, letter)?;
...
///
/// NVMe and USB-to-ATA devices are also queried through their native protocol
/// so translated identifiers and USB bridge revisions are not shown as drive
/// metadata.
fn query_storage_device_property(
    disk_number: u32,
    drive_letter: char,
) -> Option<PhysicalDriveInfo> {
    ...
    unsafe {
        ...
        let native_nvme_serial = if descriptor.bus_type == 17 {
            query_nvme_serial(handle, disk_number)
        } else {
            None
        };
        let native_usb_firmware = if descriptor.bus_type == 7 {
            query_usb_ata_firmware(handle, drive_letter)
        } else {
            None
        };

        // --- Query 2: StorageDeviceIdProperty (NVMe fallback identifier) ---
        ...
        let id_success = if descriptor.bus_type == 17 {
            DeviceIoControl(
                handle,
                IOCTL_STORAGE_QUERY_PROPERTY,
        ...
        let serial = native_nvme_serial.unwrap_or_else(|| {
            if id_success && id_bytes_returned > 0 {
                query_device_id_serial(&id_buffer, id_bytes_returned as usize)
                    .map(normalize_translated_nvme_identifier)
        ...
        Some(PhysicalDriveInfo {
            ...
            firmware_revision: native_usb_firmware.unwrap_or_else(|| firmware.trim().to_string()),
