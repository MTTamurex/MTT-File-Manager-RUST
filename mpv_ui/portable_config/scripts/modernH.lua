...
function build_keyboard_controls()
    ...
    table.insert(bottom_button_line, 'open_sub')
    table.insert(bottom_button_line, 'tog_norm')
    table.insert(bottom_button_line, 'tog_vsr')
...
function menu_open(menu_type)
    ...
    menu_add_binding('wheel_up',   'menu-wheel-up',   function() menu_navigate(-1) end, 'repeatable')
    menu_add_binding('wheel_down', 'menu-wheel-down', function() menu_navigate(1)  end, 'repeatable')
...
function osc_init()
    ...
    ne.content = function ()
        local on = mp.get_property_bool('user-data/vsr/vsr-enabled', false)
        if on then
            return '{\\fs12\\1c&H00FF00&}RTX\\NVSR{\\1c&HFFFFFF&}'
        else
            return '{\\fs12\\1c&H808080&}RTX\\NVSR{\\1c&HFFFFFF&}'
        end
    end
    ne.eventresponder['mbtn_left_up'] =
    ...
    ne.content = function ()
        local on = mp.get_property_bool('user-data/vsr/hdr-enabled', false)
        if on then
            return '{\\fs12\\1c&H00FF00&}RTX\\NHDR{\\1c&HFFFFFF&}'
        else
            return '{\\fs12\\1c&H808080&}RTX\\NHDR{\\1c&HFFFFFF&}'
        end
    end
    ne.eventresponder['mbtn_left_up'] =
    ...
    ne.visible = (osc_param.playresx >= 800)
    ne.tooltip_style = osc_styles.Tooltip
    ne.tooltipF = function ()
    ...
    ne.visible = (osc_param.playresx >= 1050)
    ne.tooltip_style = osc_styles.Tooltip
    ne.tooltipF = function ()
...
function render()
    ...
    -- Clock (upper-left corner, always visible when OSC is active)
    if state.osc_visible then
        local clock_str = os.date('%H:%M')
        ass:new_event()
        ass:an(7)
        ass:pos(14, 8)
        ass:append('{\\blur0\\bord1\\shad0\\1c&HFFFFFF&\\3c&H000000&\\fs20\\fn' .. user_opts.font .. '}')
        ass:append(clock_str)
    end
